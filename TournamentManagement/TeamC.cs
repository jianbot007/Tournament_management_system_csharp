...
{
    public partial class TeamC : Form
    {
        public TeamC()
        {
            InitializeComponent();
        ...
        private void FillPlayerInformationC()
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in PlayerdataGridView.Rows)
                {
                        ...
                        row.Cells["country"].Value != null && row.Cells["playing_role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = "INSERT INTO playerDetailsC (jersey_no, name, country, playing_role) " +
        ...
        private void RemovePlayersC()
        {
            if (PlayerdataGridView.SelectedCells.Count > 0)
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM playerDetailsC WHERE jersey_no = @jersey_no;";
        ...
        private void FillStaffsInformationC()
        {
            string connectionString = "Data Source=Najib;Initial Catalog=najib;Integrated Security=True;";
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in StaffdataGridView.Rows)
                {
                        ...
                        row.Cells["role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = @"
                    INSERT INTO staffsC (id, name, role) 
                    VALUES (@id, @name, @role);";
        ...
        private void RemoveStaffsC()
        {
            if (StaffdataGridView.SelectedCells.Count > 0) // Updated line to use StaffdataGridView
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM staffsC WHERE id = @id;";
                    SqlCommand cmd = new SqlCommand(query, con);
        ...
        {
            TeamManagmentLogin teamManagmentLogin=new TeamManagmentLogin();

            teamManagmentLogin.Show ();
            this.Close();
        }
