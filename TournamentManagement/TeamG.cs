...
{
    public partial class TeamG : Form
    {
        public TeamG()
        {
        ...
        private void FillPlayerInformationG()
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in PlayerdataGridView.Rows)
                {
                        ...
                        row.Cells["country"].Value != null && row.Cells["playing_role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = "INSERT INTO playerDetailsG (jersey_no, name, country, playing_role) " +
        ...
        private void RemovePlayersG()
        {
            if (PlayerdataGridView.SelectedCells.Count > 0)
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM playerDetailsG WHERE jersey_no = @jersey_no;";
        ...
        private void FillStaffsInformationG()
        {
            string connectionString = "Data Source=Najib;Initial Catalog=najib;Integrated Security=True;";
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in StaffdataGridView.Rows)
                {
                        ...
                        row.Cells["role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = @"
                    INSERT INTO staffsG (id, name, role) 
                    VALUES (@id, @name, @role);";
        ...
        private void RemoveStaffsG()
        {
            if (StaffdataGridView.SelectedCells.Count > 0) // Updated line to use StaffdataGridView
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM staffsG WHERE id = @id;";
                    SqlCommand cmd = new SqlCommand(query, con);
        ...
        private void TeamG_Load(object sender, EventArgs e)
        {
