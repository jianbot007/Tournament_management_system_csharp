...
{
    public partial class TeamB : Form
    {
        public TeamB()
        {
        ...
        private void FillPlayerInformationB()
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in PlayerdataGridView.Rows)
                {
                        ...
                        row.Cells["country"].Value != null && row.Cells["playing_role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = "INSERT INTO playerDetailsB (jersey_no, name, country, playing_role) " +
        ...
        private void RemovePlayersB()
        {
            if (PlayerdataGridView.SelectedCells.Count > 0)
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM playerDetailsB WHERE jersey_no = @jersey_no;";
        ...
        private void FillStaffsInformationB()
        {
            string connectionString = "Data Source=Najib;Initial Catalog=najib;Integrated Security=True;";
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in StaffdataGridView.Rows)
                {
                        ...
                        row.Cells["role"].Value != null)
                    {
                        ...
                        // Insert a new record
                        string insertQuery = @"
                    INSERT INTO staffsB (id, name, role) 
                    VALUES (@id, @name, @role);";
        ...
        private void RemoveStaffsB()
        {
            if (StaffdataGridView.SelectedCells.Count > 0) // Updated line to use StaffdataGridView
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM staffsB WHERE id = @id;";
                    SqlCommand cmd = new SqlCommand(query, con);
        ...
        private void Staffs_Click_1(object sender, EventArgs e)
        {
            ...
            FillStaffsInformationB();
        }

        private void TeamB_Load(object sender, EventArgs e)
        {
